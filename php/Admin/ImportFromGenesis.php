...
class ImportFromGenesis extends ComponentAbstract {
	...
	/**
	 * Hook suffix returned by add_submenu_page(), used to scope the
	 * admin script enqueue to this page only. False when the submenu
	 * wasn't registered (upstream plugin inactive).
	 *
	 * @var string|false
	 */
	...
	/**
	 * Enqueue the importer page's progressive-enhancement script.
	 *
	 * A static file (config travels via data-* attributes on the forms)
	 * because WordPress.org disallows raw <script> output from PHP.
	 *
	 * @param string $hook_suffix The current admin page hook suffix.
	 */
	public function enqueue_assets( $hook_suffix ) {
		if ( $this->page_hook_suffix && $hook_suffix === $this->page_hook_suffix ) {
			wp_enqueue_script(
				'coywolf-custom-blocks-import-genesis',
				$this->plugin->get_url( 'js/admin-import-genesis.js' ),
				[],
