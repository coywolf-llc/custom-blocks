 ...
 * library lands.
 *
 * When the slug resolves to nothing (empty input, a legacy slug from a
 * previous icon-set era that no longer maps to a component, or an
 * unknown name in a loaded library), this returns Lucide's
 * `LuSquareCode` — the plugin's canonical default block glyph. That
 * keeps the picker preview and the inserter from showing a blank slot
 * whenever a stored value drifts out of sync with the available icon
 * set. The fallback glyph is the one statically imported icon (it
 * tree-shakes to a couple of KB), so it is always synchronously
 * available even though every library loads lazily.
 *
 ...
 *                          `lu/LuSquareCode`, `bi/BiBox`,
 *                          `fa6/Fa6Heart`). Legacy snake_case
 *                          BoxIcons-only slugs from v1.0.10 are
 *                          accepted via `parseIconSlug`'s fallback.
 * @return {React.FunctionComponent} A renderable icon component —
 *                                   never null, falls back to
 *                                   `LuSquareCode` on every miss.
 */
const getIconComponent = ( iconSlug ) => {
	...
	// and itself falls back to LuSquareCode while loading or on
