 ...
 *
 * Current storage format: `{libKey}/{ComponentName}` — e.g. `bi/BiBox`,
 * `fa6/Fa6Bell`, `hi2/HiHome`. The component name is the literal
 * react-icons named export, no case conversion needed.
 *
 * Legacy fallbacks:
 *  - `bi_box`, `bs_arrow_down_circle` — snake_case from the 1.0.10 BoxIcons-
 *    only era. No `/` separator; we assume BoxIcons and convert
 *    snake_case → PascalCase. Works for any library whose icon names
 *    don't include digits next to letters (BoxIcons doesn't).
 *  - `coywolf_custom_blocks`, `attach_file`, etc. — pre-BoxIcons names
 *    from the original Genesis Custom Blocks set. Won't resolve; the
 *    Icon component renders nothing in that slot until the user
 *    re-picks. Acceptable per the v1.0.10 changelog note.
 *
...
export const parseIconSlug = ( slug ) => {
	...
	// Legacy snake_case → PascalCase under the default library.
	const pascal = slug
		.split( '_' )
		.map( ( part ) => part.charAt( 0 ).toUpperCase() + part.slice( 1 ) )
		.join( '' );
	return { lib: DEFAULT_LIBRARY, name: pascal };
};
