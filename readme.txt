...
= 1.0.74 =
* Harden PHP templates: never interpolate field values into PHP source (#84).

= 1.0.73 =
* Updater: run GitHub release checks in the background so the Updates screen never hangs (#83).
...
= 1.0.70 =
* WP.org-compliant templates: sandboxed interpreter + PHP execution via companion plugin (#80).
...
= 1.0.67 =
* Build deps: clear the dev-tree security advisories surfaced by the graph rescan (#77).

= 1.0.66 =
* Editor perf: lazy-load the Lucide icon library — 94% smaller block-editor bundle (#76).
...
= 1.0.63 =
* Build deps: fix all 9 GitHub Dependabot alerts (#73).
...
= 1.0.60 =
* Resolve remaining Plugin Check warnings (#70).

= 1.0.59 =
* Drop the Domain Path header (no bundled translations) (#69).

= 1.0.58 =
* CI: upgrade actions to Node 24-native versions (#67).

= 1.0.57 =
* CI: run bundled JS actions on Node 24 (#66).

= 1.0.56 =
* Use Node 26 in CI (#65).

= 1.0.55 =
* Update CI Node from 20 to 24 (#64).

= 1.0.54 =
* Trim block-icon picker to fit wp.org 10 MB limit (#63).
...
= 1.0.51 =
* Set readme Contributors to jonhenshaw (#60).

= 1.0.50 =
* Fix all WordPress.org Plugin Check errors (#59).

= 1.0.49 =
* Add WordPress.org variant build + gated SVN deploy (#58).

= 1.0.48 =
* Add Atom-feed fallback when the GitHub API is rate-limited (#57).

= 1.0.47 =
* Harden guard_pre_download: use hook_extra, fail closed (#56).

= 1.0.46 =
* Perf: honour _neg cache + cache wp_error failures in updater (#55).
