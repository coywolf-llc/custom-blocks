...
<!-- wporg-strip:start -->
* Self-updates from GitHub Releases -- the only outbound request this build
  makes is a check for new releases on the GitHub API. Plugin updates appear
  on Dashboard -> Updates and install with the standard one-click flow;
  package downloads are restricted to a GitHub-owned host allowlist.
<!-- wporg-strip:end -->
...
1. In WordPress, go to Plugins -> Add New, search for "Coywolf Custom
   Blocks", click Install Now, then Activate.
2. Go to Custom Blocks -> Add New to define your first block.
<!-- wporg-strip:start -->

Installing the GitHub build instead?
...
3. In WordPress, go to Plugins -> Add New -> Upload Plugin, upload the
   zip, and click Activate. Once installed, updates surface on
   Dashboard -> Updates.
<!-- wporg-strip:end -->
...
No. The WP Engine update server, the dormant Google Analytics client, and
the Genesis Pro upgrade nag have all been removed.
<!-- wporg-strip:start -->
The only outbound request this build makes is to api.github.com to check
for new releases of this plugin; update-package downloads are restricted to
GitHub-owned hosts.
<!-- wporg-strip:end -->
