...
#
# Eligible for WordPress.org again since 1.0.70: PHP execution moved to the
# GitHub-only "PHP Templates" companion plugin, so this plugin no longer
# executes database-stored code. The release workflow uploads the variant as
# an artifact; any actual .org submission or SVN push remains a manual,
# explicitly-confirmed step. This script doubles as the staging harness for
# local Plugin Check runs (stage the output as
# wp-content/plugins/coywolf-custom-blocks in a test install).
#
